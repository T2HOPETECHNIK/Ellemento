...
def main():
    ...
    for io_name in io_data:
        ...
        if "ARRAY" in io['type']:
            ...
            if array_type == "BOOL":
                # remove "D" and ".n" in "Dxxx.n"
                tmpstr = io['addr']
                tmp = tmpstr.split(".")
                constant_arr_addr = int(re.sub(r"\D", "", tmp[0]))
            else:
                constant_arr_addr = int(re.sub(r"\D", "", io['addr']))

            bit_addr_offset = constant_arr_addr
            bitpos = 0
            ...
            for j in range(array_size):
                ...
                if array_type == "BOOL":
                    addr = hmi_tag_plc_name + 'D' + str(bit_addr_offset) + '.' + str(bitpos)
                    raw_addr = 'D' + str(bit_addr_offset) + '.' + str(bitpos)
                    ...
                    if bitpos >= 16:
                        bitpos = 0
                        bit_addr_offset = bit_addr_offset + 1
