...
class T45ModelPlcBridge(ModelPlcBridge):
    ModelPlcDict = {}

    def __init__(self, id=-1, type_name="Default", t45_id=-1, plc_id=-1, address=None):
        super().__init__()
        self._id = id
    ...
    def part_placed(self,value):
        tag_name = self.address['write']['part_placed_S4']
        register = self.plc_io.config['address'][tag_name]
        self.plc_io.write_plctag(register[0], value, register[1])

    def part_picked(self, value):
        tag_name = self.address['write']['part_picked_S5']
        register = self.plc_io.config['address'][tag_name]
        self.plc_io.write_plctag(register[0], value, register[1])
