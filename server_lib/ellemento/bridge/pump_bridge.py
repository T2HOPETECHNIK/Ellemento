...
class PumpModelPlcBridge(ModelPlcBridge):
    ...
    def set_flowrate(self, flowrate):
        ...
        res, error = self.modbus_io.write_address_json(register_address, flowrate)
        ret = None
        if error:
            ret = res.registers[0]
        return ret, error
    ...
    def set_rpm(self, rpm):
        ...
        register_address = self.modbus_io.config['address'][tag_name][location]
        print("------", register_address)
        res, error = self.modbus_io.write_address_json(register_address, rpm)
        ret = None
        if error:
            ret = res.registers[0]
        return ret, error
    ...
    def set_fill_mode_duration(self, duration):
        ...
        register_address = self.modbus_io.config['address'][tag_name][location]
        print("------", register_address)
        res, error = self.modbus_io.write_address_json(register_address, duration)
        ret = None
        if error:
            ret = res.registers[0]
        return ret, error
